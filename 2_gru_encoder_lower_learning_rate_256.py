...
from keras.layers import Input, LSTM, Dense, Concatenate, GRU
from keras.models import Model

encoder_inputs = Input(shape=(None, 42))
encoder = GRU(256, return_sequences=True, return_state=True)
encoder_outputs, state_h= encoder(encoder_inputs)

encoder2 = GRU(256, return_sequences=True, return_state=True)
encoder_outputs1, state_h1 = encoder2(encoder_outputs)

decoder_inputs = Input(shape=(None, 59))
decoder = GRU(256, return_sequences=True, return_state=True)
...
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4), loss="mse", metrics=["accuracy", "cosine_similarity"])
...
# RESULTS
#165/165 [==============================] - 6s 35ms/step - loss: 0.0066 - accuracy: 0.1689 - cosine_similarity: 0.1929 - val_loss: 0.0067 - val_accuracy: 0.1545 - val_cosine_similarity: 0.1792
